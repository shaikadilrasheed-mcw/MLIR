# File   : test_addcmul.py
# Purpose: Implemented with scf loops, Test and compare the result of custom addcmul op and torch addcmul op
# Author : Adil
# Date   : 2026-09-24
...
def test_scalar():
    ...
    with Context():
        ...
        ref = torch.addcmul(torch.tensor(A), torch.tensor(B), torch.tensor(C)).item()
        print(f"[scalar] custom={res.value}  torch={ref}")
...
def test_tensor():
    ...
    c = np.array([ 5,  6,  7,  8], dtype=np.int32)
    ...
    with Context():
        ...
        func.func @tn(%a: tensor<4xi32>, %b: tensor<4xi32>, %c: tensor<4xi32>)
          ...
          %r = custom.addcmul %a, %b, %c : tensor<4xi32>
        ...
        ref = torch.addcmul(torch.from_numpy(a), torch.from_numpy(b),
                            torch.from_numpy(c)).numpy()
        print(f"[tensor] custom={custom.tolist()} torch={ref.tolist()}")
        assert np.array_equal(custom, ref)
        print("[tensor] MATCH ✓")
