// File   : convertCustomToTosaPass.cpp
// Purpose: Lower custom dialect ops to tosa.
// Author : Adil
// Date   : 2026-09-24
...
#define GEN_PASS_DEF_CUSTOMTOTOSA
...
struct ConvertAddCDiv : public mlir::OpConversionPattern<custom::AddCDiv> {
  using mlir::OpConversionPattern<custom::AddCDiv>::OpConversionPattern;
  mlir::LogicalResult
  matchAndRewrite(custom::AddCDiv op, OpAdaptor adaptor,
                  mlir::ConversionPatternRewriter &rewriter) const override {
    mlir::Location loc = op.getLoc();
    mlir::Value a = adaptor.getA();
    mlir::Value b = adaptor.getB();
    ...
    // ---- SCALAR case ----
    if (!mlir::isa<mlir::RankedTensorType>(a.getType())) {
      mlir::Value divideRes = rewriter.create<mlir::arith::DivSIOp>(loc, b, c);
      mlir::Value out  = rewriter.create<mlir::arith::AddIOp>(loc, a, divideRes);
      rewriter.replaceOp(op, out);
      return mlir::success();
    }
    ...
    // ---- TENSOR case ----
      ...
      // auto shiftTy = mlir::RankedTensorType::get({1},rewriter.getI8Type());
      // mlir::Value shift = rewriter.create<mlir::tosa::ConstOp>(loc,shiftTy,mlir::DenseElementsAttr::get(shiftTy,(int8_t)0));
      mlir::Value divideRes = rewriter.create<mlir::tosa::IntDivOp>(loc, type, b, c);
      mlir::Value out  = rewriter.create<mlir::tosa::AddOp>(loc, type, a, divideRes);
      rewriter.replaceOp(op, out);
      return mlir::success();
...
class CustomToTosa : public impl::CustomToTosaBase<CustomToTosa> {
public:
  ...
  void runOnOperation() final {
    ...
    patterns.add<ConvertAddCDiv>(&getContext());
...
std::unique_ptr<mlir::Pass> createCustomToTosa() {
  return std::make_unique<CustomToTosa>();
}
} // namespace custom